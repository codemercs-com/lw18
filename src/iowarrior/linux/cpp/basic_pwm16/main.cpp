/*
 * Sample code for using LED-Warrior18 with an IO-Warrior28 device (dongle, eval, or chip)
 * take a look into the Readme for install instructions for iowkit
 */
...
void print_report(IOWKIT28_SPECIAL_REPORT report, unsigned int n)
{
	std::cout << "Report: ";

	for (unsigned int i = 0; i < n; i++)
		std::cout << std::setfill('0') << std::setw(2) << std::hex << (report.Bytes[i] & 0xFF) << " ";
...
int main(int argc, char **argv)
{
	...
	char snt[9];			 // for output serialnumber
	...
	if (handle != NULL)
	{
		...
		std::cout << "Device found | PID: " << std::hex << (pid & 0xFFFF) << std::dec << " | Serial: " << snt << std::endl;
